...
public class RemoteHostConnection  extends DeploymentConstants {
	...
	public ArrayList<Object> runCommand(String runCommand) {
		
		StringBuilder outputString= new StringBuilder();
		ArrayList<Object> returnVar= new ArrayList<>();
		String line;
		try {
			// Open channel and set the command to execute on the channel and execute the command
			...
			//sleep for 3 seconds while waiting for command to execute, checking every 100 milliseconds
			int sleepCount=0;			
			do {
				Thread.sleep(100);
			}while(!channelExec.isClosed()&&sleepCount++<30);
			
			// Retrieve the exit status of the executed command
			int exitStatus=channelExec.getExitStatus();
			if(exitStatus==126)
				System.out.println("Error: Problem with permission maybe.");
			if(exitStatus==127)
				System.out.println("Error: Problem with LineFeed maybe.");
